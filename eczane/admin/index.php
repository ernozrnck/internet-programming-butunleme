...
if(isset($_POST["guncelle"])){
  $slogan = $_POST["slogan"];
  $baslik = $_POST["baslik"];
  $sql = "UPDATE `anasayfa` SET `slogan`='".$slogan."',`baslik`='".$baslik."' WHERE id = 1";
  $conn->exec($sql);
...
<body class="app">   	
					    ...
					    <ul class="app-menu footer-menu list-unstyled">
						    <li class="nav-item">
								<form action="" method="post">
									<button name="cikis"> Çıkış Yap </button>
								</form>
								<?php
								// çıkış yapma 
	...
	<div class="main">
<!-- anasayfadaki başlık ve içeriği çekiyoruz -->
	<?php
		$veri = $conn
		->query("SELECT slogan, baslik FROM anasayfa")
		->fetchAll();  
	?>
	...
	<!-- başlığı yazdırıyoruz  -->
	<input type="text" name="slogan" placeholder="Slogan" id="slogan" value="<?php print_r($veri[0][0]) ?>">
	<label for="">Başlık</label>
	<!-- içeriği yazdırıyoruz -->
	<input type="text" name="baslik" placeholder="Baslik" id="baslik" value="<?php print_r($veri[0][1]) ?>">
